...
def solve_jssp_individual_flowtime_with_fixed_ops_OLD(
    ...
    Rückgabe:
    - df_schedule: DataFrame mit ['Job','Arrival','Machine','Start','Processing Time','Flow time','End']
    """
    ...
    all_ops = []  # Liste pro Job: [(machine_id, duration), ...]
    all_machines = set()
    for job in job_list:
    ...
    for m in sorted(all_machines):
        ops_on_m = [
            (j, o, all_ops[j][o][1])
            for j in range(num_jobs)
            for o, (mach, _) in enumerate(all_ops[j])
            if mach == m
        ]
        # Konflikte neu-neu
    ...
    for j in range(num_jobs):
        ...
        prob += job_ends[j] >= starts[(j, last_o)] + all_ops[j][last_o][1]
