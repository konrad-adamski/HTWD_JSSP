...
def solve_jssp_weighted_individual_flowtime_with_fixed_ops(
    ...
    """
    Rescheduling-Variante mit fixierten Operationen und gewichteter Zielfunktion.
    ...
    Rückgabe:
    - df_schedule: DataFrame mit Schedule-Ergebnis
    - total_weighted_flowtime: gewichtete Summe der Durchlaufzeiten
    """
    # Arrival lookup & weights
    ...
    weights = {job: 1.0 / (1.0 + arrival_times[job]) for job in arrival_times}

    # Ursprüngliche Vorbereitung wie in Original-Funktion durchführen...
    # [Code wie in deiner Original-Funktion zur Variableninitialisierung übernehmen...]

    # LP-Modell
    prob = pulp.LpProblem('JSSP_WeightedIndFlow_Fixed', pulp.LpMinimize)

    # Variablen (wie vorher)
    starts = {
    ...
    # Technologische Reihenfolge und Startbedingungen, Maschinenkonflikte etc.
    # (exakt gleicher Code wie in Original-Funktion übernehmen)

    # [Code der Constraints hier identisch übernehmen]

    # Lösen
    prob.solve(pulp.HiGHS_CMD(msg=True, timeLimit=solver_time_limit))

    # Ergebnisse extrahieren (wie zuvor)
    recs = []
